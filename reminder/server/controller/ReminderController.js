...
const addReminder= async (req, res) => {
  ...
  const updateReminder=async (req, res) => {
    try {
      ...
      await list.updateOne({_id: req.params.id}, { title, description, date, time });
      const updateList=await list.findById(req.params.id);
      if (updateList) {
  ...
  const deleteReminder = async (req, res) => {
    try {
      ...
      const deleteReminder = await list.findByIdAndDelete(id);
      if (deleteReminder) {
